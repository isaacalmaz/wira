...
**Terakhir diperbarui**: 2026-09-26 (migration sampai 0080 + job CI `db-tests`; versi besar sebelumnya 2026-09-20 — baca ulang dari awal, jangan cuma diff mental dari versi lama).
...
| **CI menguji migration uang/keamanan** | 2026-09-26: job `db-tests` di `.github/workflows/ci.yml` (Postgres 16) menerapkan 0045, 0046, 0059, 0070, 0071, 0074, 0076–0080 apa adanya di atas stub Supabase kecil lalu menjalankan asersi SQL. Lihat §6.9 dan `db-tests/README.md`. |
...
6. **Jalankan 0071–0080 di Supabase kalau belum** (cek dulu, §2.4), dengan urutan deploy di baris `migrations/README.md` masing-masing. 0078 butuh pg_cron (sama seperti 0073); setelah apply cek `SELECT jobname, schedule FROM cron.job;` memuat `wira-expire-qris-orders`.
...
- **Kuota promo tidak dikembalikan saat order dibatalkan** (0076) — hanya `per_user_limit` yang mengabaikan order batal.
...
- **`db-tests` belum mencakup** dispatch (0072/0073), payout/utang komisi (0028/0075), PIN (0063–0069) dan guardrail klaim (0054) — objek-objek itu tidak ada di stub.
