-- =============================================================================
-- Migration 0001: Initial core schema — users, merchants, orders, messages
-- Source: master_schema.sql (repo root)
-- Original creation date (git history, first commit): 2026-09-07
-- =============================================================================
-- NOTE: This file's on-disk mtime (2026-09-08 20:44) is later than several
-- "fix_*" files that were logically applied on top of it (dated 2026-09-07).
-- That is because master_schema.sql was grown incrementally in place across
-- more than one session — its EARLIEST content (the users/merchants/orders
-- tables) necessarily had to exist before any file that references those
-- tables could run. It is placed first here on dependency grounds, not
-- file-timestamp grounds. See migrations/README.md for details.
--
-- NOTE: Assumes the "uuid-ossp" extension (uuid_generate_v4()) is already
-- enabled, which Supabase projects have on by default. No root-level .sql
-- file in this repo ever issues `CREATE EXTENSION "uuid-ossp"` explicitly.
-- =============================================================================
