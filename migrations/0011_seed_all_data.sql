-- =============================================================================
-- Migration 0011: locations, promos, vehicles, notifications, products tables
--                  + merchant/product/villa seed data
-- Source: seed_all_data.sql (repo root)
-- Original creation date (git history, first commit): 2026-09-11
-- =============================================================================
-- Depends on public.users (0001) and public.merchants (0001).
--
-- WARNING — CONFLICTS WITH 0010 (seed_dummy_data.sql): see 0010's header.
-- This file's notifications table (description, is_read) and products table
-- (adds category_id) use different column names than 0010's versions of the
-- same two tables. Because both use CREATE TABLE IF NOT EXISTS, only the
-- file that actually ran first defines the real column names; the other's
-- INSERTs referencing its own column names may fail. Verify against the
-- live database.
--
-- Also note: this file was originally produced by generate_sql.mjs
-- (`fs.writeFileSync('seed_all_data.sql', ...)`), but its content here has
-- diverged/grown beyond what generate_sql.mjs currently emits (e.g. its
-- notifications table still uses desc/read there, not description/is_read
-- as seen here) — generate_sql.mjs was not committed as a migration since
-- it is a one-off generator script, not something run directly against the
-- database, and it no longer matches this file's actual output.
-- =============================================================================
