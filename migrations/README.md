...
| 0014 | `0014_postgis_nearest_driver.sql` | `setup_nearest_driver.sql` | Adds spatial columns/index/trigger to `public.drivers`; creates `get_nearest_drivers()`/`find_nearest_drivers()` RPCs, RLS, `driver_locations` view. **Critical gap** — no file anywhere creates `public.drivers`; see warning in file header. |
...
1. **Missing `public.drivers` table (migration 0014).** No root `.sql` file creates it, yet `setup_nearest_driver.sql` (0014) `ALTER`s it as if it already exists, and the app's driver-matching RPCs depend on it. It was most likely created directly via the Supabase Table Editor GUI rather than SQL, or its creation script was lost. Needs to be located (or reconstructed from the live DB's actual `information_schema`) and added as a new early migration — **not invented from `backend/database/schema.sql`**, which is explicitly stale/fictional per this task's scope, even though its `drivers` table definition happens to have matching column names.

2. **`notifications`/`products` schema conflict (0010 vs 0011).** Both `seed_dummy_data.sql` and `seed_all_data.sql` do `CREATE TABLE IF NOT EXISTS` for `notifications` and `products` with different column names (`desc`/`read` vs `description`/`is_read`; products with/without `category_id`). Whichever actually ran first in production silently won, and the other file's own `INSERT`s (which reference its intended column names) may have failed silently when they were originally run. File mtimes suggest `seed_dummy_data.sql` (0010) ran first. **Needs verification against the live `information_schema.columns` for these two tables.**

3. **`seed_operational_zones.sql` (0012) references a non-existent `is_active` column** on `feature_flags`. The table as created by `fix_feature_flags_rls.sql` (0003) only has `(region, features, updated_at)`. This insert would fail against that schema unless `is_active` was added out-of-band (e.g. via the Table Editor GUI, un-captured in any `.sql` file). **Needs verification against the live schema.**
