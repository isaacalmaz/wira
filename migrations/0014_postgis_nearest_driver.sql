-- =============================================================================
-- Migration 0014: PostGIS nearest-driver matching (columns, index, trigger, RPCs)
-- Source: setup_nearest_driver.sql (repo root)
-- Original creation date (git history, first commit): 2026-09-13
-- =============================================================================
-- CRITICAL GAP — READ BEFORE REPLAYING ON A FRESH DATABASE:
-- This file ALTERs `public.drivers` and assumes that table ALREADY EXISTS,
-- but NO root-level .sql file in this repository ever issues
-- `CREATE TABLE public.drivers`. It is not the same table as
-- `public.driver_profiles` (created in migration 0007 / create_driver_profiles.sql
-- — different columns: vehicle_type/plate_number/vehicle_color/is_verified,
-- no lat/lng/geography/is_online/rating/status/vehicle_plate).
--
-- The only CREATE TABLE for `drivers` anywhere in this codebase is in
-- backend/database/schema.sql (id, vehicle_type, vehicle_plate, is_online,
-- rating, status — a column shape that does line up with what this file's
-- RPCs select), but that file is explicitly called out in this project's
-- own background as a separate, stale, mostly-fictional file that does not
-- reflect the live database, and this task was scoped to NOT touch or pull
-- from it. So rather than invent a CREATE TABLE public.drivers statement
-- (not sourced from any file that was actually run), this migration is left
-- exactly as found. On a truly fresh database this file WILL ERROR at its
-- first ALTER TABLE public.drivers statement.
--
-- ACTION NEEDED: a human familiar with the live Supabase project should
-- either (a) locate the real origin of public.drivers (most likely created
-- ad-hoc via the Supabase Table Editor GUI rather than SQL, which would
-- explain why no .sql file captures it), and add a 0013b/0014-prequel
-- migration for it, or (b) confirm backend/database/schema.sql's `drivers`
-- definition is in fact what was manually applied and adapt it explicitly
-- (as a new, clearly-labeled statement, not silently folded into this file).
-- =============================================================================
-- Also depends on the postgis extension (0013) and public.users (0001).
-- =============================================================================
