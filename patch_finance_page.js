...
// Add List to lucide-react imports if not there
...
// Add state for activeTab and transactions
code = code.replace(
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={\`px-4 py-2 rounded-lg font-semibold transition \${
            activeTab === 'requests' ? 'bg-primary text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-50'
          }\`}
        >
        ...
        <button
          ...
          className={\`px-4 py-2 rounded-lg font-semibold transition \${
            activeTab === 'transactions' ? 'bg-primary text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-50'
          }\`}
        >
...
// End requests tab and start transactions tab
const txTabHTML = `
        </div>
      )}
      ...
      {activeTab === 'transactions' && (
        <div className="card shadow-md">
          ...
          <p className="text-xs text-slate-500 mb-4">
            Menampilkan 100 riwayat transaksi uang terakhir (Top-Up, Pembayaran Pesanan, Pencairan, Koreksi).
          </p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100">
                {transactions.map(tx => {
                  // tentukan warna dan arah (tambah saldo atau kurang saldo)
                  const isIncoming = tx.type === 'topup' || tx.type === 'correction_in' || tx.type === 'refund';
